...
use sequoia::openpgp::armor;
use sequoia::openpgp::constants::DataFormat;
use sequoia::openpgp::packet::Signature;
use sequoia::openpgp::parse::stream::{
    DetachedVerifier, VerificationHelper, VerificationResult, Verifier,
};
use sequoia::openpgp::parse::{PacketParserResult, Parse};
use sequoia::openpgp::serialize::stream::{
    EncryptionMode, Encryptor, LiteralWriter, Message, Signer,
};
use sequoia::openpgp::serialize::Serialize;
use sequoia::openpgp::{Error, KeyID, Packet, Result, TPK};
use sequoia::store;
...
pub fn encrypt(
...

    {
        ...
        let message = Message::new(&mut crypted_data);

        // We want to encrypt a literal data packet.
        let mut sink = Encryptor::new(message, &[], &recipients, EncryptionMode::ForTransport)
...
pub fn sign(
    ...
    secrets: Vec<sequoia::openpgp::TPK>,
    detached: bool,
    binary: bool,
    append: bool,
    notarize: bool,
    ...
    match (detached, append | notarize) {
        (_, false) | (true, true) => {
            sign_data(input, output_path, secrets, detached, binary, append, force)
        }
        (false, true) => sign_message(input, output_path, secrets, binary, notarize, force),
    }
}
...
fn sign_data(
    ...
    let (mut output, prepend_sigs, tmp_path): (Box<io::Write>, Vec<Signature>, Option<PathBuf>) =
        if detached && append && output_path.is_some() {
            // First, read the existing signatures.
            let mut sigs = Vec::new();
            let mut ppr = sequoia::openpgp::parse::PacketParser::from_file(output_path.unwrap())?;
            ...
            while let PacketParserResult::Some(mut pp) = ppr {
                ...
                match packet {
                    ...
                    p => {
                        return Err(
                            failure::err_msg(format!("{} in detached signature", p.tag()))
            ...
            let tmp_file = NamedTempFile::new_in(
                PathBuf::from(output_path.unwrap())
                    .parent()
    ...
    let keys: Vec<&sequoia::openpgp::TPK> = secrets.iter().collect();
    ...
    if let Some(path) = tmp_path {
        ...
        fs::rename(path, output_path.expect("must be Some if tmp_path is Some"))?;
    }
    Ok(())
}
...
fn sign_message(
    ...
    let mut ppr = sequoia::openpgp::parse::PacketParser::from_reader(input)
    ...
    while let PacketParserResult::Some(mut pp) = ppr {
        if !pp.possible_message() {
            return Err(Error::MalformedMessage("Malformed OpenPGP message".into()).into());
        }
        ...
        match state {
            State::AfterFirstSigGroup => {
                ...
                sink = Signer::new(sink, &keys).context("Failed to create signer")?;
            ...
            State::Signing { signature_count } if signature_count == 0 => {
                ...
                sink = sink.finalize_one().context("Failed to sign data")?.unwrap();
                state = State::Done;
            }
        ...
        if let Packet::Literal(_) = pp.packet {
            ...
            // message packet.
            let mut literal =
                LiteralWriter::new(sink, l.format(), l.filename(), l.date().map(|d| *d))
                    .context("Failed to create literal writer")?;

            // Finally, just copy all the data.
            io::copy(&mut pp, &mut literal).context("Failed to sign data")?;
        ...
        match packet {
            ...
            Packet::Signature(ref sig) => {
                sig.serialize(&mut sink).context("Failed to serialize")?;
                if let State::Signing {
                    ref mut signature_count,
                } = state
                {
                    *signature_count -= 1;
    ...
    match state {
        State::Signing { signature_count } => {
            ...
            sink.finalize_one().context("Failed to sign data")?.unwrap();
        }
        State::Done => (),
...
impl<'a> VerificationHelper for VHelper<'a> {
    fn get_public_keys(&mut self, ids: &[KeyID]) -> Result<Vec<TPK>> {
        ...
        let seen: HashSet<_> = tpks
            .iter()
            .flat_map(|tpk| tpk.keys().map(|(_, key)| key.fingerprint().to_keyid()))
            .collect();
    ...
    fn check(&mut self, sigs: Vec<Vec<VerificationResult>>) -> Result<()> {
        ...
        for (i, results) in sigs.into_iter().enumerate() {
            for result in results {
                ...
                let trusted = issuer
                    .as_ref()
                    .map(|i| self.trusted.contains(&i))
                    .unwrap_or(false);
                let what = match (i == 0, trusted) {
                    (true, true) => "signature".into(),
                    (false, true) => format!("level {} notarization", i),
                    (true, false) => "checksum".into(),
                    (false, false) => format!("level {} notarizing checksum", i),
                };

                match result {
                    GoodChecksum(_) => {
                        let issuer = issuer.expect("good checksum has an issuer");
                    ...
                    MissingKey(_) => {
                        let issuer = issuer.expect("missing key checksum has an issuer");
                    ...
                    BadChecksum(_) => {
                        if let Some(issuer) = issuer {
                            ...
                            eprintln!("Bad {} without issuer information", what);
                        }
                        if trusted {
        ...
        if self.good_signatures >= self.signatures && self.bad_signatures + self.bad_checksums == 0
        {
            Ok(())
        } else {
            self.print_status();
...
pub fn split(input: &mut io::Read, prefix: &str) -> Result<()> {
    // We (ab)use the mapping feature to create byte-accurate dumps of
    // nested packets.
    let mut ppr = sequoia::openpgp::parse::PacketParserBuilder::from_reader(input)?
        .map(true)
        .finalize()?;
    ...
    while let PacketParserResult::Some(pp) = ppr {
        if let Some(ref map) = pp.map() {
            ...
            let mut sink = File::create(filename).context("Failed to create output file")?;
        ...
        match old_depth.cmp(&new_depth) {
            Ordering::Less => pos.push(0),
            Ordering::Equal => *pos.last_mut().unwrap() += 1,
            Ordering::Greater => {
                pos.pop();
            }
        }
