...
impl PubKeyVaultHandler {
    pub fn new(recipient: String) -> PubKeyVaultHandler {
        PubKeyVaultHandler {
            recipient: recipient,
        }
    }
...
impl VaultHandler for PubKeyVaultHandler {
    fn encrypt(&self, open_vault: UnsealedVault) -> Result<SealedVault> {
        Ok(SealedVault {
            secret: encrypt(open_vault.plain_secret, self.recipient.to_owned())?,
    ...
    fn decrypt(&self, open_vault: SealedVault) -> Result<UnsealedVault> {
        Ok(UnsealedVault {
            plain_secret: String::from_utf8(decrypt(open_vault.secret)?).unwrap(),
            format: open_vault.format,
        })
    }
}

pub fn handle(command: &ArgMatches) -> Result<()> {
...
pub fn import_key(gpg_key: String) -> Result<String> {
...
pub fn parse_private_key() -> Result<UserConfig> {
...
pub fn verify(content: String, signature: String) -> Result<bool> {
  let content_file = cmd!("mktemp").read()?;
  let signature_file = cmd!("mktemp").read()?;
  cmd!("echo", &content).stdout(&content_file).run()?;
  cmd!("echo", &signature).pipe(cmd!("base64", "-d").stdout(&signature_file)).run()?;

  match cmd!("gpg", "--homedir=.culper_gpg", "--verify", &signature_file, &content_file).stdout_capture().stderr_capture().run() {
    Ok(outcome) => {
    ...
    _ => Err(ErrorKind::RuntimeError("Could not spawn GPG process for verifying payload.".to_owned()).into())
  }
}

pub fn encrypt(plain: String, recipient: String) -> Result<Vec<u8>> {
    let mut child = Command::new("gpg")
        .arg("--homedir=.culper_gpg")
        .arg("--encrypt")
        .arg("-r")
...
pub fn decrypt(secret: Vec<u8>) -> Result<Vec<u8>> {
    let mut child = Command::new("gpg")
        .arg("--homedir=.culper_gpg")
        .arg("--decrypt")
        .stdin(Stdio::piped())
